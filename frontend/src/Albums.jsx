  ...
  return (
    <div className="relative min-w-[200px]">
      <input
        ...
        min="1000-01-01"
        max="9999-12-31"
        className="w-full px-3 py-2 rounded-full border border-gray-700 bg-gray-800 text-sm text-white placeholder-transparent"
      />

      {/* Show placeholder or formatted value overlay when input is not focused */}
      {!focused && (
        <div className="pointer-events-none absolute left-4 top-1/2 -translate-y-1/2 italic text-gray-300 text-sm">
...
export default function Albums() {
  console.log('Rendering Albums component');
  const [albums, setAlbums] = useState([]);
  const [allGenres, setAllGenres] = useState([]);
  const [loading, setLoading] = useState(false);
  const { filters, setFilters, handleFilterChange, toggleGenre, resetFilters, clearAllGenres, removeFilterChip } = useContext(FilterContext);
  ...
  const albumsLengthRef = useRef(0);

  useEffect(() => {
    async function checkAuthAndLoad() {
      console.log('Checking cache/auth in Albums...');
      ...
      try {
        ...
        if (isOnline && (isStale || cachedAlbums.length === 0)) {
          ...
          if (fullResp.ok) {
            ...
            setTotalCount(updatedAlbums.length);
          }
        } else if (!isOnline) {
        ...
        console.error('Error in Albums load:', err);
        if (!navigator.onLine) {
  ...
  useEffect(() => {
    const callback = (entries) => {
      if (entries[0].isIntersecting && !loadingRef.current) {
        if (albumsLengthRef.current < totalCount) {
          console.log('Loading more albums, current page:', pageRef.current, 'loaded:', albumsLengthRef.current, 'total:', totalCount);
          setPage((prev) => prev + 1);
        }
      }
    };
  ...
  async function loadAlbums(pageToLoad = 1) {
    ...
    try {
      ...
      if (searchQuery.trim()) {const sq = searchQuery.toLowerCase(); albums = albums.filter(a => a.album_name.toLowerCase().includes(sq) || a.artists.toLowerCase().includes(sq));}
      ...
      setTotalPages(Math.ceil(albums.length / limit));

      const paginated = albums.slice((pageToLoad - 1) * limit, pageToLoad * limit);
      setAlbums(prev => {
        if (pageToLoad === 1) {
          return paginated;
        } else {
          const existingIds = new Set(prev.map(a => a.album_id));
          const newAlbums = paginated.filter(a => !existingIds.has(a.album_id));
          return [...prev, ...newAlbums];
        }
      });
      setHasMore(pageToLoad * limit < albums.length);
    } catch (err) {
  ...
  try {
      ...
      <>
        ...
        >
          <div className="max-w-7xl mx-auto p-4">
            <h1 className="text-4xl font-bold mb-6 text-center">Liked Albums</h1>
            ...
            <div className="flex flex-wrap gap-4 mb-6 justify-center items-center">
              <button onClick={handleLogout} className="rounded-full bg-green-600 text-black px-5 py-2 font-semibold hover:bg-red-700 shadow-md cursor-pointer border border-white/30" disabled={loading || isRefreshing} aria-label="Logout">
                Logout
              </button>

              <button onClick={handleRefresh} className="rounded-full bg-purple-800 text-white px-5 py-2 font-semibold hover:bg-yellow-700 shadow-md transition cursor-pointer border border-white/30" disabled={loading || isRefreshing} aria-label="Refresh albums from server">
                Refresh Albums
              </button>
              ...
              <button
                ...
                onClick={() => navigate("/shuffle", { state: { filters } })}
                aria-label="Go to shuffle mode"
              >
                Shuffle
              </button>
            ...
            {/* Filters container – removed top helper text as requested */}
            <motion.div
              initial={false}
              animate={{ height: showFilters ? "auto" : 0, opacity: showFilters ? 1 : 0 }}
              transition={{ duration: 0.25 }}
              className={`mb-6 relative z-10 ${showFilters ? 'overflow-visible' : 'overflow-hidden'}`}
            >
              <div className="bg-gray-800 p-4 rounded-2xl shadow-2xl border border-gray-700">
                <div className="flex flex-wrap gap-4 items-end">
                  {/* Genre multi-select (styled dropdown) */}
                  <div className="relative w-full sm:w-auto" ref={genreDropdownRef}>
                    ...
                    {showGenres && (
                      <div className="absolute z-[60] mt-2 bg-gray-800 border border-gray-700 rounded-2xl p-3 w-56 max-h-64 overflow-auto shadow-2xl">
                  ...
                  <div className="w-full sm:w-auto">
                    <label htmlFor="minTracks" className="sr-only">Min Tracks</label>
                    <input
                      id="minTracks"
                  ...
                  <div className="w-full sm:w-auto">
                    <label htmlFor="maxTracks" className="sr-only">Max Tracks</label>
                    <input
                      id="maxTracks"
                  ...
                  {/* Date inputs (faux placeholder handled inside component) */}
                  <div className="w-full sm:w-auto">
                    <DateSelector
                  ...
                  <div className="w-full sm:w-auto">
                    ...
                    <select
                      ...
                      className="px-4 py-2 rounded-full border border-gray-700 bg-gray-800 w-full sm:w-36 text-sm appearance-none cursor-pointer"
                      aria-label="Number of albums per page"
                    >
                      {[10, 25, 50, 100].map(l => (
            ...
            {/* Summary - stadium design sized to content, left above albums */}
            <div className="mb-4">
            ...
            {/* Album grid */}
            <motion.div 
              className="grid gap-4 grid-cols-1 xs:grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-5 mb-6 transition-all duration-300" 
              role="list" 
            ...
            >
              {albums.length === 0 && totalCount === 0 && !loading && (
                <div className="col-span-full text-center text-gray-300">No albums match your filters</div>
              )}

              {albums.map(a => {
                const min = Math.floor(a.total_duration / 60);
                const sec = Math.floor(a.total_duration % 60);
                ...
                return (
                  <motion.div 
                    ...
                    role="listitem" 
                    tabIndex={0} 
                    className="bg-gray-800 rounded-2xl overflow-hidden flex flex-col hover:bg-gray-700 transition transform hover:-translate-y-0.5 hover:scale-[1.01] shadow-md cursor-pointer"
                  ...
                  >
                    <a 
                      ...
                      rel="noreferrer" 
                      className="block"
                      aria-label={`Open ${a.album_name} by ${a.artists} on Spotify`}
                    >
                      <img 
                        src={a.image_url} 
                        alt={`Cover of ${a.album_name} by ${a.artists}`} 
                    ...
                    <div className="p-3 flex flex-col gap-1">
                      ...
                      <div className="text-gray-300 text-sm truncate">
                        {a.artists.split("; ").map((artist, i) => (
                          <span key={i}>
                            <a
                              href={a.artist_urls.split("; ")[i]}
                              target="_blank"
                              rel="noreferrer"
                            ...
                            {i < a.artists.split("; ").length - 1 ? ", " : ""}
                          </span>
                        ))}
                      </div>
                      <div className="flex items-center justify-between gap-2">
                        <div className="text-gray-500 text-xs flex-1">
                          {a.release_date} • {a.total_tracks} tracks • {durationStr} • Added: {a.added_at.split("T")[0]}
                        </div>
            ...
            <button
              ...
              className="fixed bottom-8 right-8 flex items-center justify-center w-12 h-12 rounded-full bg-green-500 border border-white/30 text-black text-2xl shadow-lg hover:shadow-xl hover:bg-green-600 transition-all duration-300 z-40 cursor-pointer backdrop-blur-sm"
    ...
    return <div>Error rendering Albums page</div>;
  }
}